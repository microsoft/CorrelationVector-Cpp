...
return {impl::convert_to_array(g)};
}

std::string guid::to_string() const
{
    return utilities::to_hex_str(m_bytes.data(), 0, 4) + '-' + utilities::to_hex_str(m_bytes.data(), 4, 2) + '-' +
           utilities::to_hex_str(m_bytes.data(), 6, 2) + '-' + utilities::to_hex_str(m_bytes.data(), 8, 2) + '-' +
           utilities::to_hex_str(m_bytes.data(), 10, 6);
}

static constexpr const unsigned char base64_table[65] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
